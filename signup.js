document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", (e) => {
        ...
        // Basic validation for each input
        if (!firstName) {
            showError("first-name-error", "First Name is required.");
            isValid = false;
        } else if (/[^a-zA-Z]/.test(firstName)) { // Only letters
        ...
        if (!lastName) {
            ...
            isValid = false;
        } else if (/[^a-zA-Z]/.test(lastName)) { // Only letters
            showError("last-name-error", "Last Name should only contain letters.");
            isValid = false;
        }

        // Validate email format (allow anything, as long as it's a valid email)
        if (!validateEmail(email)) {
            showError("email-error", "Please enter a valid email.");
            isValid = false;
        }

        // Check if the first name and last name are reflected in the email
        ...
        if (isValid) {
            const hashedPassword = hashPassword(password); // Hash password during signup
            localStorage.setItem("userEmail", email);
            localStorage.setItem("userPasswordHash", hashedPassword); // Store hashed password
